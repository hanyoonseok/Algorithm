...
`골드1` `구현` `시뮬레이션` 
---
- 코드트리의 [2개의 사탕](https://www.codetree.ai/training-field/frequent-problems/two-candies/description?page=1&pageSize=20&username=hys6078) 문제와 동일하다. 
- 출력 양식에 차이만 있을 뿐 코드 그대로 제출해도 솔브됨...
- 이동 후에 파란 구슬과 빨간 구슬의 위치에 따라서 백트래킹 진행
- 방향에 따라, 그리고 어떤 구슬이 어디에 위치한지에 따라 분기가 많아지기 때문에, 이를 처리해주는게 관건
- 두 구슬이 한번에 구멍에 들어가는 경우도 파악해야함
