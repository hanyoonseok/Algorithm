# 백준 #15644 [구슬 탈출3](https://www.acmicpc.net/problem/15644)
`골드1` `백 트래킹` `구현` `BFS`
---
- [구슬 탈출2](https://www.acmicpc.net/problem/13460) 문제에서 도출 경로를 출력하는 것이 추가된 문제
- 구슬이 탈출하는 과정을 backtrack 파라미터로 관리해서 정답에 같이 갱신해줌으로 해결했음

### 전체코드
```java
public class B15644_구슬탈출3 {

	static int N,M;
	static char map[][];
	static int d[][] = {{1,0}, {0,1}, {-1, 0}, {0, -1}};
	static char routes [] = {'D','R','U','L'};
	static int memo [][];
	...
	static boolean backtrack(int cnt, int [] red, int [] blue, int prev, String route) {
		...
		for(int i=0; i<4; i++) {
			...
			if(backtrack(cnt+1, new int [] {nextPos[0], nextPos[1]}, new int [] {nextPos[2], nextPos[3]}, i, route+routes[i])) continue;
		}
