# 코드트리 [2개의 사탕](https://www.codetree.ai/training-field/frequent-problems/two-candies/description?page=1&pageSize=20&username=hys6078)
`골드1` `백트래킹` `구현` `시뮬레이션`
---
- 조건 처리해야할 조건이 많고, 백트래킹 구현 자체가 어려워서 까다로웠던 문제
- 매 트래킹마다 맵 상태를 복사하여 조건을 검사해야 했음
- 메모이제이션을 통해서 왔던 좌표를 불 필요하게 재 방문하는 경우를 제어해야 했음
- 같은 방향으로 움직이더라도 빨간 사탕이 오른쪽에 있는지, 왼쪽에 있는지를 다시 한번 더 검사해야 하고, 이에 따른 검사가 달라짐
- 2일 걸려서 겨우 솔브...ㅜ
